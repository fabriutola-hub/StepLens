...
flowchart LR
  ...
  SDK["@agent-replay/sdk<br/>(or wrappers)"]
  Studio[("StepLens Studio<br/>(Next.js API)")]
  DB[("SQLite<br/>~/.agent-replay/studio.db")]
  UI["Studio Workbench UI<br/>(React + Tailwind)"]
  CLI["steplens CLI"]
  Otel["OTel collector<br/>(optional)"]

  Agent -- "run/record" --> SDK
  SDK -- "POST /api/ingest" --> Studio
  Studio -- "Drizzle ORM" --> DB
  Studio -- "JSON API" --> UI
  Studio -- "JSON API" --> CLI
  Studio -- "OTLP/HTTP" --> Otel

  CLI -- "GET /api/..." --> Studio
  Otel -- "jaeger / tempo /<br/>datadog / etc." --> Users(("Operators"))

  classDef external fill:#e8f4ff,stroke:#3b82f6
  classDef internal fill:#f3f4f6,stroke:#9ca3af
  classDef storage  fill:#fef3c7,stroke:#d97706

  class Agent,Users,Otel external
  class SDK,Studio,UI,CLI internal
  class DB storage
```

## Local live updates (SSE)

The Workbench can poll every 3s (default) or subscribe to push-based
...
sequenceDiagram
  ...
  Browser->>Studio: GET /api/events/stream
  Studio-->>Browser: event: hello {now: 1717…}
  loop every 1s
    Studio->>SQLite: SELECT WHERE startedAt >= lastSeen
    alt new traces
      Studio-->>Browser: event: trace {id, name, status, startedAt}
    end
    Studio-->>Browser: event: ping {}
  end
  Browser->>Browser: invalidate list query
```

Polling is the default for clients that prefer simpler connection
management; SSE is the default for the Workbench when "Live updates" is
on (Settings → Workbench).

## Settings & theming

UI preferences (theme, language, live updates, budget alerts) live in
the browser under `localStorage["steplens.settings.v1"]`. The Settings
panel at `/settings` is the canonical editor; for tooling, write
directly to the same key (the schema is in
`apps/studio/src/lib/settings-store.ts`).

Server-side, the only "settings" are environment variables consumed at
boot — see [README.md](../README.md) for the list.
